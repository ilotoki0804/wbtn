...
class Webtoon:
    ...
    def __init__(
        ...
        self.content = WebtoonContentManger(self)
    ...
    def _merge(
        ...
        with self.connection.cursor() as cur, self.connection.cursor() as updater:
            ...
            updater.execute("""
                INSERT OR REPLACE INTO dest.Episode (episode_no, added_at)
                SELECT episode_no, added_at FROM main.Episode
            """)

            updater.execute("""
                INSERT OR REPLACE INTO dest.EpisodeInfo (episode_no, kind, conversion, value)
                SELECT episode_no, kind, conversion, value FROM main.EpisodeInfo
            """)

            updater.execute("""
                INSERT OR REPLACE INTO dest.Content (episode_no, content_no, kind, data, conversion, path, added_at)
                SELECT episode_no, content_no, kind, data, conversion, path, added_at FROM main.Content
            """)
